...
grep -Fq 'max by (node) ((rate(node_cpu_usage_seconds_total' "$work_dir/rendered.yaml"
grep -Fq 'max by (node) ((node_memory_working_set_bytes' "$work_dir/rendered.yaml"
...
grep -q '^  minAvailable: 1$' "$work_dir/rendered.yaml"
